...
namespace Day_7
{
    ...
    public class Program
    {
        ...
        public static (Node,int) GetGoldNode(string input = null)
        {
            ...
            foreach (var descriptor in descriptors)
            {
                ...
                var nodeToBeAdded = !NodeExistsAnywhereInExistingNodeInfrastructure(newNode, nodes);
                if (nodeToBeAdded)
                {
                    nodes.Add((newNode, 0));
                }

                var parentNode = nodes.First(x => x.Item1.Colour == newNode.Colour && x.Item1.Pattern == newNode.Pattern);

                foreach (var child in newNode.Children)
                {
                    if (!NodeExistsAnywhereInExistingNodeInfrastructure(child.Item1, nodes))
                    {
                        var newChildNode = new Node
                        {
                            Pattern = child.Item1.Pattern,
                        ...
                        if (nodeToBeAdded)
                        {
                            parentNode.Item1.Children = parentNode.Item1.Children.Skip(1);
                        }
                    }
                    else
                    {
                        var existingNode = nodes.Where(x => x.Item1.Pattern == child.Item1.Pattern && x.Item1.Colour == child.Item1.Colour).First();

                        existingNode.Item1.Parents = existingNode.Item1.Parents.Append((parentNode.Item1, 0));
                        parentNode.Item1.Children = parentNode.Item1.Children.Append((existingNode.Item1, child.Item2));
                        if (nodeToBeAdded)
                        {
                            parentNode.Item1.Children = parentNode.Item1.Children.Skip(1);
                        }
        ...
        public static int GetNumberOfKids(Node node)
        {
            ...
            {
                var numberOfKids = GetNumberOfKids(child.Item1);
                count += (numberOfKids + 1) * child.Item2;
            }
        ...
        public static IEnumerable<string> GetNParents(Node node)
        {
            ...
            bagColours.AddRange(node.Parents.Select(x => x.Item1.Colour + x.Item1.Pattern));
            foreach (var parent in node.Parents)
            {
                bagColours.AddRange(GetNParents(parent.Item1));
            }
        ...
        public static IEnumerable<(Node, int)> FindMatchingNodesInNodeInfrastructure(Node node, IEnumerable<(Node, int)> existingNodes)
        {
            ...
            {
                if (node.Colour == existingNode.Item1.Colour && node.Pattern == existingNode.Item1.Pattern)
                {
                    matchingNodes.Add(existingNode);
                }

                if (existingNode.Item1.Children.Any())
                {
                    matchingNodes.AddRange(FindMatchingNodesInNodeInfrastructure(node, existingNode.Item1.Children));
                }
            }
    ...
    public static class LineParser
    {
        public static Node Parse(string input)
        {
            ...
            var indexOfContain = input.IndexOf(" contain ");
            var parentBagDescriptor = input.Substring(0, indexOfContain);

            var parentBagPattern = parentBagDescriptor.Substring(0, parentBagDescriptor.IndexOf(" "));
            var parentBagColourWithBags = parentBagDescriptor.Substring(parentBagDescriptor.IndexOf(" ") + 1);
            var parentBagColour = parentBagColourWithBags.Substring(0, parentBagColourWithBags.IndexOf(" ")); // I hope there isn't a baggy colour

            var indexOfFirstBag = indexOfContain + 9;

            var childBagDescriptors = input
                .Substring(indexOfFirstBag)
                .Split(", ");

            var node = new Node
            {
                Colour = parentBagColour,
                Pattern = parentBagPattern,
                Children = new List<(Node, int)>()
            };

            foreach (var descriptor in childBagDescriptors)
            {
                if (descriptor.Substring(0, 2) == "no")
                {
                    continue;
                }

                var number = int.Parse(descriptor.Substring(0, descriptor.IndexOf(" ")));
                var patternAndColour = descriptor.Substring(descriptor.IndexOf(" ") + 1, descriptor.IndexOf("bag") - descriptor.IndexOf(" ") - 2);
                var pattern = patternAndColour.Substring(0, patternAndColour.IndexOf(" "));
                var colour = patternAndColour.Substring(patternAndColour.IndexOf(" ") + 1);

                node.Children = node.Children.Append((new Node
                {
                    Pattern = pattern,
                    Colour = colour,
                    Parents = new List<(Node, int)> { (node, 0) }
                }, number));
            }
